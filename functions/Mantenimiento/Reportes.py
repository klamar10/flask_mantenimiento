...
def reporte_excel_RxT():
    try:
        ...
        for row in consulta:
            ...
            sh.write(idx + 1, 2, str(row['MT_Enombre']))
...
def List_Reporte2():
    ...
    if request.method == 'POST':
        ...
        consulta = MT_areas.query.filter(MT_areas.MT_Aid == area,
                                         ...
                                         .join(MT_funcion_resp, MT_funcion_resp.MT_ASFid == MT_asig_funciones.MT_ASFid)
                                         .filter(MT_asig_et_fn.MT_Eid == area,
...
def List_Reporte3():
    if request.method == 'POST':
        ...
        cur.execute('''
                SELECT x.MT_Anombre, a.MT_ABnombre,f.MT_Enombre,
                ...
                WHERE c.MT_ASFfech_asigdesde >= %s and c.MT_ASFfech_asighasta <= %s 
                ORDER BY x.MT_Anombre, a.MT_ABnombre, d.MT_ASFfech_crea''', (fini, ffin))
        data = cur.fetchall()
        cur.close()
...
def ReporteTotal3():
    ...
    sh.write(5, 8, '=($I$5/CONTAR(d2:d5000))*100%')
    idx = 0
